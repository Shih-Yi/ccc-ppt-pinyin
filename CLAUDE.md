...
- **間距是固定值,UI 不開放調**:寫死 `DEFAULT_GAP_PCT = 90`(佔中文字級的百分比)。48pt 歌詞配 20pt 拼音實測,90 是「拼音貼在中文正下方但不相碰」的下限,70 以下兩行就疊在一起,所以沒有留調整空間、也就不放進 Advanced settings。要改就改 `pinyin_pptx.py` 的常數;`add_pinyin(gap_pct=...)` 參數仍在(填 0 = 完全不寫 `lnSpc`、維持原檔行距),給換檔案時排查用。拼音段落只會被歸零 `spcBef`(那段空白也落在這條縫裡),行高一律不寫,所以拼音下面那條縫維持原檔兩行中文之間的距離。
...
- **內建讀音修正**:`祢 → nǐ`、`的 → di`(唱的時候不是輕聲 de,刻意不標調號)、`尊主為大 → wéi`。程式裡 `CHAR_OVERRIDES` / `PHRASE_OVERRIDES` 兩個 dict 可自行擴充。單字覆寫只蓋單字,詞庫裡的詞不受影響——例如「目的地」仍是 `mù dì dì`。
...
- **間距沒有調整餘裕**:48pt 歌詞 + 20pt 拼音的相碰點約在 75%,固定值 90 已接近極限。若把 Pinyin font size 調大(例如 20 → 26pt),相碰點會往上移,90 就可能開始撞到——這時要回去把 `DEFAULT_GAP_PCT` 調高。
...
3. 中文與拼音的間距是固定的(90%),UI 上沒有這個選項;若這份檔看起來太擠或太鬆,改 `pinyin_pptx.py` 的 `DEFAULT_GAP_PCT`。
