...
- **間距是固定值,UI 不開放調**:寫死 `DEFAULT_GAP_PCT = 95`(佔中文字級的百分比)。48pt 歌詞配 20pt 拼音實測,相碰點約在 75%、70 以下兩行整個疊在一起,可用範圍很窄,所以不放進 Advanced settings。要改就改 `pinyin_pptx.py` 的常數;`add_pinyin(gap_pct=...)` 參數仍在(填 0 = 完全不寫 `lnSpc`、維持原檔行距),給換檔案時排查用。拼音段落只會被歸零 `spcBef`(那段空白也落在這條縫裡),行高一律不寫,所以拼音下面那條縫維持原檔兩行中文之間的距離。
...
- **間距沒有調整餘裕**:48pt 歌詞 + 20pt 拼音的相碰點約在 75%,固定值 95 離極限不遠。若把 Pinyin font size 調大(例如 20 → 26pt),相碰點會往上移,95 就可能開始撞到——這時要回去把 `DEFAULT_GAP_PCT` 調高。
...
3. 中文與拼音的間距是固定的(95%),UI 上沒有這個選項;若這份檔看起來太擠或太鬆,改 `pinyin_pptx.py` 的 `DEFAULT_GAP_PCT`。
