...
- **中文與拼音的間距**:拼音段落會寫入自己的行高(`<a:lnSpc><a:spcPct>`,預設 50%,並把 `spcBef` 歸零),在 Advanced settings 的「Gap between lyric and its pinyin (%)」可調(20–200%)。播放軟體是依「該行自己的行高」往下推進,所以縮小拼音行的行高就等於把那行拼音往上貼近它對應的中文行;中文行與中文行之間的行距完全不動。
