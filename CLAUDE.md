...
- **中文與拼音的間距**:拼音段落會寫入自己的行高(`<a:lnSpc><a:spcPct>`,預設 70%,並把 `spcBef` 歸零),在 Advanced settings 的「Gap between lyric and its pinyin (%)」可調(20–200%)。播放軟體是依「該行自己的行高」往下推進,所以縮小拼音行的行高就等於把那行拼音往上貼近它對應的中文行;中文行與中文行之間的行距完全不動。
...
- **間距太小會相碰**:百分比低於約 100% 時拼音會往中文行靠,調到 40% 以下、或拼音字級偏大時,可能碰到中文字的下緣。發現相碰就把百分比往上調。
- **調間距可能觸發 PowerPoint 的自動縮放**:歌詞文字框若開了「文字溢出時自動縮小」(`<a:normAutofit fontScale=...>`),拼音行變矮會讓整塊文字變短,PowerPoint 重新排版時就把 fontScale 往上跳一級(級距約 7–10%),結果整個文字框裡的中文和英文一起變大、位置全跑掉。預設 70% 是目前實測不會跳級的值;要再調更緊之前,先確認該文字框的 autofit 設定。
...
3. 覺得拼音跟中文離太遠或太擠,調 Advanced settings 的 Gap between lyric and its pinyin。
