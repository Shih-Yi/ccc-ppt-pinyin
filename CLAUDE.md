...
- **間距設定**:Advanced settings 的「Gap between lyric and its pinyin (%)」,0–200%,以中文字級的百分比計。**預設 90%**——48pt 歌詞配 20pt 拼音實測,90 是「拼音貼在中文正下方但不相碰」的下限,70 以下兩行就疊在一起。**填 0 則完全不寫 `lnSpc`**,維持原檔自己的行距(換一份行距不同的檔案時的退路)。拼音段落只會被歸零 `spcBef`(那段空白也落在這條縫裡),行高一律不寫,所以拼音下面那條縫維持原檔兩行中文之間的距離。
...
- **間距太小會相碰**:百分比壓太低時,拼音會撞進中文字的下緣。48pt 歌詞 + 20pt 拼音的相碰點約在 75%,所以預設 90 已接近極限;拼音字級改大,可用的下限會跟著往上移。
...
3. 覺得拼音跟中文離太遠,把 Advanced settings 的 Gap between lyric and its pinyin 從 90 往下調;相碰就往上調回來。換到行距明顯不同的檔案,先填 0 看原樣再決定。
