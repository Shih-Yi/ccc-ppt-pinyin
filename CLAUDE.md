...
- **間距設定**:Advanced settings 的「Gap between lyric and its pinyin (%)」,0–200%,以中文字級的百分比計。**預設 0 = 完全不動原檔的行距**,因為每份檔原本的行距不同,寫死一個百分比可能反而變鬆。要收緊就從 0 往下試(例如先填 60,再往下調)。拼音段落只會被歸零 `spcBef`(那段空白也落在這條縫裡),行高一律不寫,所以拼音下面那條縫維持原檔兩行中文之間的距離。
...
3. 覺得拼音跟中文離太遠,把 Advanced settings 的 Gap between lyric and its pinyin 從 0 改成一個百分比(例如 60)再往下試;調到相碰就往上調回來。
